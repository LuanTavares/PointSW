...
bool ProducaoDAO::existeEstaProducao(Producao * pro) {
    Producao * dialogPro = new Producao(pro);
    bool retorno = false;

    if(db.open()) {
        query = QSqlQuery(db);
        query.prepare("SELECT op FROM Producao WHERE CodigoMaquina = ? AND op = ?");
        query.addBindValue(dialogPro->getMaquina()->getCodigoMaquina());
        query.addBindValue(dialogPro->getOP()->getOP());
        if(!query.exec()){
            std::cout << query.lastError().text().toStdString() << std::endl;
            db.close();
            retorno = false;
